...
SV * _create_result(SV* con_sv, drizzle_result_st* result_raw, drizzle_query_st *query_raw) {
    net_sth * sth;
    Newxz(sth, 1, net_sth);
    sth->drizzle = SvREFCNT_inc_simple(XS_STATE(net_con*, con_sv)->drizzle);
    sth->con     = SvREFCNT_inc_simple(con_sv);
    sth->result  = result_raw;
    sth->query   = query_raw;
    return _bless("Net::Drizzle::Result", sth);
}
...
PPCODE:
    ...
    XPUSHi(ret);
    XPUSHs(_create_result(self, result, NULL));
    XSRETURN(2);
...
CODE:
    ...
    RETVAL = _create_result(_self, result, NULL);
OUTPUT:
    RETVAL
...
CODE:
    ...
    RETVAL = _create_result(_self, NULL, query_d);
OUTPUT:
    RETVAL
